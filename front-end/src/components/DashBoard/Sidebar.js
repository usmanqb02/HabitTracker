...
const SidebarItem = ({ icon, text, onClick, isSidebarOpen, isActive }) => (
  <button
    ...
    className={`flex items-center space-x-3 text-lg text-gray-200 p-2 rounded-md transition-all duration-200 w-full 
      ${isActive ? 'bg-teal-800' : 'hover:bg-gradient-to-r hover:from-teal-500 hover:to-cyan-500'}`}
    aria-label={text}
  >
...
const menuItems = [
  ...
  { icon: "M12 6v6l4 2M12 6l-4 2", text: "Manage Habits", path: "manageHabits" },
  { icon: "M5 3v10h3v8h8v-8h3V3H5z", text: "Tracking", path: "tracking" },
  { icon: "M4 6h16M4 12h16M4 18h16", text: "Goals", path: "goals" },
...
const Sidebar = ({ currentPath }) => {
  ...
  const handleNavigate = (path) => {
    navigate(`/${path}`); // Navigating to the correct route
  };

  return (
    <div className={`flex ${isSidebarOpen ? 'w-65' : 'w-21'} transition-all duration-300 ease-in-out`}>
      <div className="bg-gradient-to-r from-teal-600 to-cyan-500 text-white shadow-lg h-screen p-4">
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" className={`h-6 w-6 ${!isSidebarOpen && 'rotate-180'} transition-transform duration-300`}>
